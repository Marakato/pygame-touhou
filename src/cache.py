...
def cache_rotate(img,theta,flag=False):
	...
	if image.has_key(img) == False:
		cache_buffer(img)


	if image[img][0].has_key(theta):
...
def cache_set_mask(img,color,flag=False):
	if hue.has_key(img)==False:
		hue[img]={}
	if hue[img].has_key(color)==False:
		if flag==True:
			...
			tp=img.copy()
			tp.fill(color,Nne,BLEND_RGB_ADD)
			return tp
	else:
		return hue[img][color]